...
C<set> value (from a previous F<config.sh> or Configure -D option)
...
D:F</bin/sh.exe>.
This unit comes before F<Options.U>, so you can't set sh with a -D
option, though you can override this (and startsh)
with -O -Dsh=F</bin/whatever> -Dstartsh=whatever

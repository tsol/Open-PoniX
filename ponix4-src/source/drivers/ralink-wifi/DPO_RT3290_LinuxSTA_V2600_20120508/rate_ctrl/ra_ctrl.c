...
// Item No.   Mode   Curr-MCS   TrainUp   TrainDown		// Mode- Bit0: STBC, Bit1: Short GI, Bit4,5: Mode(0:CCK, 1:OFDM, 2:HT Mix, 3:HT GF)
	0x0c, 0x0b,  0,  0,  0,						// Initial used item after association
	0x00, 0x00,  0, 40, 101,
	0x01, 0x00,  1, 40, 50,
	0x02, 0x00,  2, 35, 45,
	0x03, 0x00,  3, 20, 45,
	...
	IN CHAR		RssiOffset)
{
	...
	{/*  N mode with 1 stream */
		UCHAR _64QAMRssiOffset = 0;

		if (IS_RT3290LE(pAd))
			_64QAMRssiOffset = 4;
		if (mcs[7]>=0 && (Rssi > (-72+RssiOffset+_64QAMRssiOffset)) && (pEntry->SupportHTMCS[MCS_7]))
			TxRateIdx = mcs[7];
		else if (mcs[6]>=0 && (Rssi > (-74+RssiOffset+_64QAMRssiOffset)) && (pEntry->SupportHTMCS[MCS_6]))
			TxRateIdx = mcs[6];
		else if (mcs[5]>=0 && (Rssi > (-77+RssiOffset+_64QAMRssiOffset)) && (pEntry->SupportHTMCS[MCS_5]))
			TxRateIdx = mcs[5];
	...
	{/*  Legacy mode */
			...
			if (IS_RT3290LE(pAd))
			{
				...
				if (mcs[7]>=0 && (Rssi > -70+RssiOffset) && (pEntry->SupportOFDMMCS[MCS_7]))
				TxRateIdx = mcs[7];
				else if (mcs[6]>=0 && (Rssi > -74+RssiOffset) && (pEntry->SupportOFDMMCS[MCS_7]))
					TxRateIdx = mcs[6];
				else if (mcs[5]>=0 && (Rssi > -78+RssiOffset) && (pEntry->SupportOFDMMCS[MCS_7]))
					TxRateIdx = mcs[5];
				else if (mcs[4]>=0 && (Rssi > -82+RssiOffset) && (pEntry->SupportOFDMMCS[MCS_7]))
					TxRateIdx = mcs[4];
				else if (mcs[4] == -1)							/*  for B-only mode */
				{
					if (mcs[3]>=0 && (Rssi > -85+RssiOffset) && (pEntry->SupportCCKMCS[MCS_3]))
						TxRateIdx = mcs[3];
					else if (mcs[2]>=0 && (Rssi > -87+RssiOffset) && (pEntry->SupportCCKMCS[MCS_2]))
						TxRateIdx = mcs[2];
					else if (mcs[1]>=0 && (Rssi > -90+RssiOffset) && (pEntry->SupportCCKMCS[MCS_1]))
						TxRateIdx = mcs[1];

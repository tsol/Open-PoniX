	...
	IN PRTMP_ADAPTER pAd)
{
   	...
   	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++)
	{
		...
		if (TxTotalCnt)
		{
			...
			if (IS_RT3290(pAd) &&  ((AccuTxTotalCnt > 150) || (pAd->AntennaDiversityInfo.AntennaDiversityState == 1)) && (pAd->CommonCfg.BBPCurrentBW == BW_40))
			{
...
*/
	...
	/*		if (TxRateIdx != pAd->CommonCfg.TxRateIndex) */
		...
		{
			if(pAd->AntennaDiversityInfo.TrainCounter < 50)
				pAd->AntennaDiversityInfo.TrainCounter = 50;
		}
		else if (pEntry->CurrTxRateIndex < CurrRateIdx)
		{
			if(pAd->AntennaDiversityInfo.TrainCounter < 20)
				pAd->AntennaDiversityInfo.TrainCounter = 20;			
		}
#endif

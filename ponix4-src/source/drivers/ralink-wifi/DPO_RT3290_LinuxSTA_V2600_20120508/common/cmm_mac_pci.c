	...
	IN BOOLEAN       bFromTx)
{
   ...
   /* RTMP_CLEAR_PSFLAG(pAd, fRTMP_PS_GO_TO_SLEEP_NOW);*/
	...
	if (IS_RT3290(pAd))
	{
		DBGPRINT(RT_DEBUG_TRACE, ("RT3290 PSM :=======RT28xxPciStaAsicForceWakeup===  \n"));
	}
	else
#endif /* RT3290 */
    	if (bFromTx == TRUE
        ...
        if (RT28xxPciAsicRadioOn(pAd, DOT11POWERSAVE))
        {
			...
			{
				// Just stop 480MHz clock of digital circuit and it will not affect RF circuit.
			}
#endif
			else
			{
...
#endif /* PCIE_PS_SUPPORT */
    {
        ...
        AsicSendCommandToMcu(pAd, WAKE_MCU_CMD, 0xff, 0x00, 0x02, FALSE);
        AutoWakeupCfg.word = 0;
	    RTMP_IO_WRITE32(pAd, AUTO_WAKEUP_CFG, AutoWakeupCfg.word);
    }
	...
	IN USHORT           TbttNumToNextWakeUp) 
{
	...
	if (pAd->OpMode == OPMODE_AP && Level==DOT11POWERSAVE)
		return FALSE;
#ifdef RT3290
	if (IS_RT3290(pAd) && (Level == GUIRADIO_OFF))
	{
		// Don't check 
	}
	else
...
#ifdef RT3290
		// We should not sleep when we are going to connect BT
		if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_BT_4WAY))
		{
			DBGPRINT(RT_DEBUG_TRACE, ("PSM :AsicRadioOff return on fOP_STATUS_BT_4WAY flag\n"));			
			return FALSE;
		}
#endif
	}
    ...
    /* When PCI clock is off, don't want to service interrupt.*/
		...
		if (IS_RT3290(pAd))
		{
			RTMP_IO_WRITE32(pAd, INT_MASK_CSR, 0);	
		}
		else 
#endif
			RTMP_IO_WRITE32(pAd, INT_MASK_CSR, AutoWakeupInt);
	...
	if (IS_RT3290(pAd))
	{
		RTMP_IO_WRITE32(pAd, INT_SOURCE_CSR, 0x7fffffff);
	}
	else
#endif
	RTMP_IO_WRITE32(pAd, RX_CRX_IDX, pAd->RxRing.RxCpuIdx);
...
/*OPSTATUS_SET_FLAG(pAd, fOP_STATUS_CLKSELECT_40MHZ);*/
	...
	if (Level == DOT11POWERSAVE
#ifdef RT3290 
|| ((Level == GUIRADIO_OFF)&&(IS_RT3290(pAd)))
#endif /* RT3290 */
	)
	{
		AUTO_WAKEUP_STRUC	AutoWakeupCfg;
...
{    
        ...
        &&(pAd->StaCfg.PSControl.field.EnableNewPS == TRUE))
    {
    	...
    	RTMPPCIeLinkCtrlValueRestore(pAd, RESTORE_WAKEUP);
#ifdef RT3290
	CmdStatus = AsicSendCommandToMcu(pAd, 0x31, PowerWakeCID, 0x00, 0x02, FALSE);
	pAd->bPCIclkOff = FALSE;
	RTMP_CLEAR_PSFLAG(pAd, fRTMP_PS_SET_PCI_CLK_OFF_COMMAND);	// set at AsicRadioOff, Clear here
	RetryRound = 0;
	do
	{
		if (CmdStatus)
		{
			// Wait command ok.
			CmdStatus = AsicCheckCommanOk(pAd, PowerWakeCID);
		}

		if (CmdStatus)
			break;      // PowerWakeCID cmd succeeded

		DBGPRINT(RT_DEBUG_TRACE, ("PSM :WakeUp Cmd Failed, retry %d\n", RetryRound));
	...
	if (RetryRound > 6)
		DBGPRINT(RT_DEBUG_ERROR, ("PSM :ASIC 0x31 WakeUp Cmd may Fail\n"));

	...
	IN PRTMP_ADAPTER pAd)
{
	...
	UCHAR	RF_R24_Value = 0,tx_agc_fc=0;
 ...
 if (IS_RT3290(pAd))
{
            ...
            pAd->Mlme.CaliBW40RfR24 = 0x30; //Bit[5] must be 1 for BW 40      
            ...
            do 
            {
                    ...
                    {
                            //
                            // tx_h20M[0] tx_agc_gc[7]
                            //
                            RT30xxReadRFRegister(pAd, RF_R30, &value);
                            value &=~(0x02); //tx_h20M[0]
                            RT30xxWriteRFRegister(pAd, RF_R30, value);
                    ...
                    //Enable BB loopback
                    //
                    RT30xxReadRFRegister(pAd, RF_R36, &value);
                    value |= 0x02;
                    RT30xxWriteRFRegister(pAd, RF_R36, value);                       

                    // Write 0x00 to BBP_R24 to set power & frequency of passband test tone
                    value = 0;
                    RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R158, 2);
                    RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, value);                      

                    //
                    //Baseband loop back TX filter bandwidth calibration - R0<2:0>[2], Enable RF calibration - R0<7>[1]
                    //
                    value = 0x82;
                    RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R158, 0);
                    RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, value);
                    ...
                    } while ((ReTry++ < 100) && ((value & 0x80)==0x80));


                    // Read BBP_R55[6:0] for received power, set R55x = BBP_R55[6:0]
                    //RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R55, &value);
                    RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R158, BBP_R57);
                    RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R159, &value);                             
                    R55x = value & 0xFF;

                    DBGPRINT(RT_DEBUG_TRACE, ("ReTry = %d, R55x = 0x%x\n",ReTry, R55x));
                    
                    // Write 0x06 to BBP_R24 to set power & frequency of stopband test tones
                    value = 6;
                    RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R158, 2);
                    RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, value);                              
                    ...
                    while(TRUE)
                    {
                            ...
                            DBGPRINT(RT_DEBUG_TRACE, ("RTMPFilterCalibration loopcnt=%d, BBP_R55=0x%x, tx_agc_fc=0x%x, R55x=0x%x, FilterTarget=0x%x, (R55x - value)=0x%x\n",
                                    loopcnt, value, tx_agc_fc, R55x, FilterTarget, (R55x - value)));
                    ...
                    if (loopcnt < 100)
                    {
                            ...
                            {
                                    //BandWidth = 20 MHz
                                    pAd->Mlme.CaliBW20RfR24 = (UCHAR)tx_agc_fc;
                            }
                            else
                            {
                                    //BandWidth = 40 MHz
                                    pAd->Mlme.CaliBW40RfR24 = (UCHAR)tx_agc_fc;
                                    pAd->Mlme.CaliBW40RfR24 |= 0x20; //be careful
                                    break;
                            }
                    }
                    else

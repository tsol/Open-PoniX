	...
	case CNTL_WAIT_OID_LIST_SCAN:
		if (Elem->MsgType == MT2_SCAN_CONF) {
			...
			pAd->Mlme.CntlMachine.CurrState = CNTL_IDLE;
				...
				if (IS_RT3290(pAd))
				{
					//for Software Select Dominate Antenna 	
					if (SSDA_TEST_FLAG(pAd, fSSDA_ENABLE))
					{
						SelectDominateAntenna(pAd);
					}
					//for Software Select Dominate Antenna - MlmeradioOn
					if(SSDA_TEST_FLAG(pAd, fSSDA_ENABLE))
					{
						if(SSDA_TEST_FLAG(pAd, fSSDA_RADIO_ON_EXEC))
						{
							ForceSelectDominateAntenna(pAd);
							SSDA_CLEAR_FLAG(pAd, fSSDA_RADIO_ON_EXEC);
						}
					}
				}
#endif /*RT3290*/
...
/*			RTMPWPARemoveAllKeys(pAd);*/
	...
	if ((IS_RT3290(pAd) && (pAd->CmbCtrl.field.AUX_OPT_Bit15_Two_AntennaMode == FALSE)))
	{
		...
		RTMP_IO_WRITE32(pAd, PBF_MAX_PCNT, 0x1F3F7F9F);
		DBGPRINT(RT_DEBUG_TRACE, ("Txburst 3290\n"));
	}
	else
#endif

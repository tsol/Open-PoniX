...
#define DRV_MODULE_VERSION	\
	__stringify(TG3_MAJ_NUM) "." __stringify(TG3_MIN_NUM) "c"
#define DRV_MODULE_RELDATE	"Aug 14, 2012"
...
static int tg3_setup_fiber_mii_phy(struct tg3 *tp, int force_reset)
{
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5719 ||
		...
		if (!tg3_readphy(tp, 0x14, &sgsr) && (sgsr & 0x1)) {
			tp->mac_mode &= ~MAC_MODE_PORT_MODE_MASK;
			if (sgsr & 0x10) {
...
static int tg3_start(struct tg3 *tp, bool reset_phy)
{
	 ...
	 */
	if (tp->dev->features & NETIF_F_LOOPBACK)

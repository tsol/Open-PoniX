...
if grep -q "SKBTX_HW_TSTAMP" $srcdir/include/linux/skbuff.h ; then
...
if [ -f $srcdir/include/linux/netdev_features.h ]; then
	if grep -q "netdev_features_t" $srcdir/include/linux/netdev_features.h ; then
...
if [ -f $srcdif/include/linux/mdio.h ]; then
...
if [ -f $srcdir/include/linux/timecompare.h -a \
     -f $srcdir/include/linux/net_tstamp.h ]; then

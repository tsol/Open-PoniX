...
--enable-os2=no --enable-beos=no --enable-glitz=no --enable-directfb=no --enable-ps=no \
--enable-pdf=no --enable-svg=no --enable-gtk-doc=no --enable-test-surfaces=no --with-x

...
dnl $Aumix: aumix/acinclude.m4,v 1.3 2002/10/29 21:27:50 trevor Exp $
...
dnl AM_PATH_GTK1([MINIMUM-VERSION, [ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND [, MODULES]]]])
dnl Test for GTK, and define GTK_CFLAGS and GTK_LIBS
dnl
AC_DEFUN(AM_PATH_GTK1,
[dnl 
dnl Get the cflags and libraries from the gtk-config script
dnl
AC_ARG_WITH(gtk1-prefix,[  --with-gtk1-prefix=PFX  Prefix where GTK 1.2 is installed (optional)],
            gtk_config_prefix="$withval", gtk_config_prefix="")
AC_ARG_WITH(gtk1-exec-prefix,[  --with-gtk1-exec-prefix=PFX Exec prefix where GTK 1.2 is installed (optional)],
            gtk_config_exec_prefix="$withval", gtk_config_exec_prefix="")
AC_ARG_ENABLE(gtk1test, [  --disable-gtk1test      Do not try to compile and run a test GTK 1.2 program],
		    , enable_gtk1test=yes)
  ...
  AC_MSG_CHECKING(for GTK 1.2 - version >= $min_gtk_version)
  no_gtk=""
     ...
     if test "$GTK_CONFIG" = "no" ; then
       ...
       if test -f conf.gtktest ; then
        :
          ...
          echo "*** Could not run GTK 1.2 test program, checking why..."
          CFLAGS="$CFLAGS $GTK_CFLAGS"
          LIBS="$LIBS $GTK_LIBS"
          AC_TRY_LINK([
#include <gtk/gtk.h>

...
*/
	...
	for (i = 0; i < sizeof(LANGUAGES) / sizeof(*LANGUAGES); i++) {
		if ((lang = (char *) getenv("LANG")) && !strncmp(lang, LANGUAGES[i], 5)) {
			menu_width = MENUSIZES[i];
			break;
		}
	}
	...
	for (i = 0; i < SOUND_MIXER_NRDEVICES; i++) {
		if ((1 << i) & devmask) {
			attrset(COLOR_PAIR(AXIS_COLOR));
			/* draw control labels */
			mvaddstr(YOFFSET + y++, XOFFSET + menu_width + R_P_WIDTH + level_width + ARROW_WIDTH, LOCAL_TEXT(dev_label[i]));
		}
	}
	PlaceCursor();
	RefreshAllSettings();
...
int             InitColors(char *scheme)
{
	...
	int             bg, fg, i;
	if ((strlen(scheme) + strlen(DATADIR)) >= PATH_MAX)
...
void            EraseLevelCurses(int dev)
{
	...
	for (i = 0; i < level_width; i++)
		mvaddch(YOFFSET + y, XOFFSET + menu_width + R_P_WIDTH + i, '+');
	PlaceCursor();
}

 ...
 * - Line 196: to make the menu accessible by keyboard, change to display the context menu by setting X and Y values depending on the selected resource
 * - Lines 231-240: restricting focus to cycle only beetween context menu elements
 */
(function ($) {
    $.contextMenu = {
        shadow: true,
        ...
        createMenu: function (menu, cmenu) {
            ...
            var $t = $('<table class="contextMenuPop" cellspacing=0 cellpadding=0></table>').click(function () { cmenu.hide(); return false; }); // We wrap a table around it so width can be flexible
            var $tr = $('<tr></tr>');
            var $td = $('<td></td>');
            ...
            var $topRow = $('<tr class="contextMenuTopRow"><td class="corner topLeft"/><td class="top"/><td class="corner topRight"/></tr>');
            $t.append($topRow);

            $tr.append($('<td class="left"/>'));
            $tr.append($td.append($div));
            $tr.append($('<td class="right"/>'));
            $t.append($tr);

            var $bottomRow = $('<tr class="contextMenuTopRow"><td class="corner bottomLeft"/><td class="bottom"/><td class="corner bottomRight"/></tr>');
            $t.append($bottomRow);
        ...
        createMenuItem: function (label, obj) {
            ...
            var $idiv = $('<div class="' + cmenu.innerDivClassName + '" style="' + iconStyle + '"><a href="#">' + label + '</a></div>');
            $div.append($idiv);
            return $div;
        },
        ...
        show: function (t, e) {
            // RECEIVER: if using the keyboard then there X and Y, so just display it in the middle of the resource that has focus
            var cmenu = this, x = e.pageX ? e.pageX : $(t).offset().left, y = e.pageY ? e.pageY : $(t).offset().top;
            cmenu.target = t; // Preserve the object that triggered this context menu so menu item click methods can see it       

            // beforeShow is a way to prevent menu from appearing (beforeShow = false means don't show menu)
            // so in fact code in beforeShow is executed at all times in the *if* statement
            if (cmenu.beforeShow() !== false) {
                ...
                // Cannot do it in beforeShow block as DOM element must be visible to perform operations
                $c.ctxsBindFocusWithin().ctxsEnableArrowNavigation().ctxsUnifyHoverFocus({ hoverClass: "context-menu-item-hover", selector: ".context-menu-item" });
                $c.find(":tabbable:first").focus();

                $(document).one('click.hidemenu', null, function () {
                    $(cmenu.menu).ctxsUnbindFocusWithin().ctxsDisableArrowNavigation().ctxsUnbindHoverFocusEvents();
                    cmenu.hide();

...
LPCSTR gAddinPath1 = FREERDP_ADDIN_PATH;
LPCSTR gInstallPrefix1 = FREERDP_INSTALL_PREFIX;
...
FREERDP_ADDIN** freerdp_channels_list_dynamic_addins(LPSTR pszName, LPSTR pszSubsystem, LPSTR pszType, DWORD dwFlags)
{
	...
	cchAddinPath = strlen(gAddinPath1);
	cchInstallPrefix = strlen(gInstallPrefix1);
	...
	CopyMemory(pszSearchPath, gInstallPrefix1, cchInstallPrefix);
	pszSearchPath[cchInstallPrefix] = '\0';

	NativePathCchAppendA(pszSearchPath, cchSearchPath + 1, gAddinPath1);

...
void xf_SendClientEvent(xfInfo* xfi, xfWindow* window, Atom atom, unsigned int numArgs, ...)
{
       ...
       xevent.xclient.serial = xevent.xany.send_event;
...
void xf_SetWindowFullscreen(xfInfo* xfi, xfWindow* window, boolean fullscreen)
{
	if (fullscreen)
	{
		...
		/* harry - this is for JWM */
		int prop = xfi->_NET_WM_STATE_FULLSCREEN;
		XChangeProperty(xfi->display, window->handle,
			xfi->_NET_WM_STATE, XA_ATOM, 32, PropModeAppend, (unsigned char *) &prop, 1);
...
/*
		...
		xev.xclient.message_type=xfi->_NET_WM_STATE;
		xev.xclient.format=32;
		xev.xclient.data.l[0] = 1; // add
		xev.xclient.data.l[1] = xfi->_NET_WM_STATE_FULLSCREEN;
		xev.xclient.data.l[2] = 0;

		XSendEvent(xfi->display, DefaultRootWindow(xfi->display), False,
			SubstructureRedirectMask | SubstructureNotifyMask, &xev);
*/
		xf_SendClientEvent(xfi, window, xfi->_NET_WM_STATE, 3, 1, xfi->_NET_WM_STATE_FULLSCREEN, 0);

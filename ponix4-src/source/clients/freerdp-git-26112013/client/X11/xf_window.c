...
void xf_SetWindowFullscreen(xfContext* xfc, xfWindow* window, BOOL fullscreen)
{
	if (fullscreen)
	{
		...
		XMoveResizeWindow(xfc->display, window->handle, settings->DesktopPosX, settings->DesktopPosY, window->width, window->height);
                ...
                int prop = xfc->_NET_WM_STATE_FULLSCREEN;
                XChangeProperty(xfc->display, window->handle,
                           xfc->_NET_WM_STATE, XA_ATOM, 32, PropModeAppend, (unsigned char *) &prop, 1);
                           
                xf_SendClientEvent(xfc, window, xfc->_NET_WM_STATE, 3, 1, xfc->_NET_WM_STATE_FULLSCREEN, 0);
                /* /harry */

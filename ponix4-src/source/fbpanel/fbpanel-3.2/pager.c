...
desk_new(pager *pg, int i)
{
    ...
    /*    
    g_signal_connect (G_OBJECT (d->da), "map",
          (GCallback) desk_map_event, (gpointer)p);
    
    g_signal_connect (G_OBJECT (d->da), "realize",
          (GCallback) desk_realize_event, (gpointer)p);
    g_object_set_data(GTK_OBJECT(d->da), "mykey", (gpointer)i);
	*/
...
pager_constructor(plugin *p)
{
    ...
    gdk_window_add_filter(gdk_get_default_root_window(), (GdkFilterFunc)pager_event_filter, pg);
    RET(1);
}
...
pager_destructor(plugin *p)
{
    ...
    gdk_window_remove_filter(gdk_get_default_root_window(), (GdkFilterFunc)pager_event_filter, pg);
    while (--pg->desknum) {

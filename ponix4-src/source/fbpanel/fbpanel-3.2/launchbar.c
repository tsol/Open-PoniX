...
launchbar_destructor(plugin *p)
{
    ...
    for (i = 0; i < lb->btn_num; i++) {
        ...
        g_object_unref(lb->btns[i].mask);
        g_object_unref(lb->btns[i].pix);
    }
    g_free(lb);
    RET();
}
...
read_button(plugin *p)
{
    ...
    if (!gp) {
        ...
        ratio = (p->panel->orientation == ORIENT_HORIZ) ?
            (float) (p->panel->ah ) / (float) gdk_pixbuf_get_height(gp)
            : (float) (p->panel->aw) / (float) gdk_pixbuf_get_width(gp);
        gps =  gdk_pixbuf_scale_simple (gp,
              ratio * ((float) gdk_pixbuf_get_width(gp) - 2),
              ratio * ((float) gdk_pixbuf_get_height(gp) - 2),
              GDK_INTERP_HYPER);
    ...
    */
    ...
    g_signal_connect (G_OBJECT (button), "clicked",
                        G_CALLBACK (clicked), (gpointer) &lb->btns[lb->btn_num]);

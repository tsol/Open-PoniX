...
#if 0
static void
tk_callback_state_changed (GtkWidget *widget, GtkStateType state, task *tk)
{
    GtkStateType reqstate;
    ENTER;
    reqstate = (tk->focused) ? tk->tb->focused_state : tk->tb->normal_state;
    if (state != reqstate)
...
tk_build_gui(taskbar *tb, task *tk)
{
    ...
    //g_signal_connect_after (G_OBJECT (tk->button), "state-changed",
    //     G_CALLBACK (tk_callback_state_changed), (gpointer) tk);
    /* pix and name */
...
tb_net_client_list(GtkWidget *widget, taskbar *tb)
{
    ...
    for (i = 0; i < tb->win_num; i++) {
        if ((tk = g_hash_table_lookup(tb->task_list, &tb->wins[i]))) {
            ...
            DBG("adding %08x(%p) %s\n", tk->win, gdk_window_lookup(tk->win), tk->name);
        }
    }

...
{
    GtkBgboxPrivate *priv = GTK_BGBOX_GET_PRIVATE (widget);
    
    ENTER;
    priv->style = widget->style;
...
static void
gtk_bgbox_size_request (GtkWidget      *widget,
...
gtk_bgbox_size_allocate (GtkWidget *widget, GtkAllocation *wa)
{
    ...
    */
    ...
    if (GTK_WIDGET_NO_WINDOW (widget)) {
        ca.x = wa->x + GTK_CONTAINER (widget)->border_width;
        ca.y = wa->y + GTK_CONTAINER (widget)->border_width;
    } else {
        ca.x = 0;
        ca.y = 0;
    }
    ca.width  = MAX (wa->width  - GTK_CONTAINER (widget)->border_width * 2, 0);
    ca.height = MAX (wa->height - GTK_CONTAINER (widget)->border_width * 2, 0);
...
static void
gtk_bgbox_free_bg(GtkWidget *widget)
{
    GtkBgboxPrivate *priv;
    ...
    if (priv->bg_type == BG_ROOT) {
        ...
        DBG2("bg root 4\n");
        priv->pixmap = NULL;
        g_signal_handlers_disconnect_by_func(priv->bg, gtk_bgbox_bg_changed, widget);
...
static GtkStyle *
gtk_bgbox_get_root_style(GtkWidget *widget)
{
    GtkBgboxPrivate *priv = GTK_BGBOX_GET_PRIVATE (widget);
    GtkStyle *style;
    int i;
    ...
    style = gtk_style_copy(priv->style);
    for (i = 0; i < 5; i++) 
        style->bg_pixmap[i] = priv->pixmap;
...
gtk_bgbox_set_background(GtkWidget *widget, int bg_type, guint32 tintcolor, gint alpha)
{
    ...
    if (priv->bg_type == BG_STYLE) {
        ...
        priv->alpha = alpha;
        style = gtk_bgbox_get_root_style(widget);
    } else {
        style = NULL;
    }
...
void
gtk_bgbox_set_bgm(GtkWidget *widget, FbBg *bg)
{
    GtkBgboxPrivate *priv;
    ENTER;
    g_warning("gtk_bgbox_set_bgm is depreceted\n");
    RET();
    priv = GTK_BGBOX_GET_PRIVATE (widget);
    priv->bg = bg;
}

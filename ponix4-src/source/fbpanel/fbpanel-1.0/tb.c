/* tb.c -- taskbar module of fbpanel project
 *
...
static int accept_skip_pager = 1;
...
static GdkPixmap *gen_pixmap;
static GdkBitmap *gen_mask;
static taskbar tb;

static void display_tk(taskbar *tb, task *tk);
static void tb_propertynotify(XEvent *ev);
static GdkFilterReturn tb_event_filter( GdkXEvent *, GdkEvent *, void *);
static void read_wmpix(FILE *fp);



#define TASK_VISIBLE(tb, tk) \
 ((tk)->desktop == (tb)->my_desktop || (tk)->desktop == -1 /* 0xFFFFFFFF */ \
     || (tk)->sticky)

#define TASK_MANAGABLE(tk) \
 (!((tk)->skip_taskbar || (accept_skip_pager && (tk)->skip_pager))) 
...
load_config(FILE *fp)
{
	...
	if (!strncmp(str, tt, strlen(tt))) {
            tb.tooltips = !strncmp(g_strstrip(str + strlen(tt)), "on", 2);
            //printf("tb.tooltips = %d\n", tb.tooltips);
        } else if (!strcmp(str, "[")) {
            read_wmpix(fp);
        } else {
	    g_warning("unknown option %s\n", str);
	    return;
	}
...
read_wmpix(FILE *fp)
{
    ...
    if (!gp) {
        fprintf(stderr, "taskbar: can't read pixmap %s\n", fname);
        return;
    }
...
adjust_task_width(taskbar * tb)
{
    ...
    //DBG( "dno=%d, task_width=%d hbox_width=%d\n", dno, tb->task_width, tb->hbox_width);
}
...
static void
callback_clicked( GtkWidget *widget, gpointer   data )
{
    task *tk = (task *) data;

    //DBG( "cb: %x: ico=%d focus=%d\n", tk->win, tk->iconified, tk->focused);
    if (tk->iconified)    {
	//tk->iconified = 0;
...
static void
simple_expose( GtkWidget *widget, gpointer   data )
{
...
add_task(taskbar * tb, task *tk)
{
    ...
    gtk_box_pack_start(GTK_BOX(tb->hbox), tk->eb, FALSE, FALSE, 0);
    gtk_signal_connect(GTK_OBJECT(tk->eb), "expose_event",
	GTK_SIGNAL_FUNC(gtk_widget_queue_draw), NULL);
    
    tk->sep = gtk_vseparator_new();
    gtk_widget_show(tk->sep);
    gtk_container_add (GTK_CONTAINER (tk->eb), tk->sep);
...
do_net_client_list()
{
    ...
    printf("%03d:M: ", nnn);
    for (list = tb.task_list; list; list = list->next)
        printf("%x ", list->win);
    printf("\n");
    nnn++;
    */
	...
	if (!find_task (&tb, win[j])) {
	    ...
	    if (win[j] != topxwin && TASK_MANAGABLE(&tkk)) {
		tk = alloc_task(win[j]);
		tk->sticky = tkk.sticky;
                tk->iconified = tkk.iconified;
                tk->hidden = tkk.hidden;
		add_task (&tb, tk);
	    }
	}
    }
...
do_net_active_window()
{
    ...
    if (otk && (!tk || (otk->win != tk->win))) {
        ...
        display_tk(&tb, otk);
        /*
          DBG( "do_net_active_window: %x: ico=%d focus=%d\n",
...
tb_propertynotify(XEvent *ev)
{
	...
	if (at == atom__NET_WM_DESKTOP) {
	    ...
	    //DBG( "atom_WM_STATE tk->iconified=%d\n", tk->iconified);
	    tk->iconified = is_iconified (tk->win);
	    gtk_label_set_text (GTK_LABEL(tk->label), tk->iconified ? tk->iname : tk->name);
...
static void
hbox_size_cb( GtkWidget *widget, gpointer   data )
{

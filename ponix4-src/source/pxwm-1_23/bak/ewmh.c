...
is_dialog:
	...
	else if(type==types->splash)
	{
		/* FIXME:  Remove area that would have been consumed
		   by the titlebar.  */ 
		c->flags |= AR_CLIENT_DONT_USE_TITLEBAR
#ifdef SHAPED
			| AR_CLIENT_SHAPED
#endif /* SHAPED */
			;
...
arwm_handle_net_wm_state_hints(Client * c)
{
	unsigned long nitems;
#if 0
	unsigned char * state_prop = arwm_get_property(c->window, 
		GETATOM("_NET_WM_STATE"), XA_ATOM, &nitems);
	Atom state;
#endif 
	unsigned char * prop;
	...
	
	{
		...
		LOG_DEBUG("\tget std_hints");

		std_hints=XGetWMHints(arwm.X.dpy, w);
		if(std_hints)
		{
			long flags = std_hints->flags;

			LOG_DEBUG("\t\ttest std_hints");
	...
	if(prop)
	{
		XFree(prop);
handle_urgent:
		LOG_DEBUG("URGENT window detected, flashing");

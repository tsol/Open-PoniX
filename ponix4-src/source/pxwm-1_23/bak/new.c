...
#endif */
//		get_mouse_position(&x, &y, c->screen->root);
//		c->geometry.x = (x * (xmax - c->border - c->geometry.width)) / xmax;
//		c->geometry.y = (y * (ymax - c->border - c->geometry.height)) / ymax;


                // center window by default
		c->geometry.x = (xmax - c->border - c->geometry.width) / 2;
		c->geometry.y = (ymax - c->border - c->geometry.height) / 2;
		send_config(c);
	}

...
{
#ifdef USE_XFT
	arwm.X.font = XftFontOpenName(arwm.X.dpy, DefaultScreen(arwm.X.dpy),
		arwm_opt_font);
#else /* ! USE_XFT */
	arwm.X.font = XLoadQueryFont(arwm.X.dpy, arwm_opt_font);
	if (!arwm.X.font) 
		arwm.X.font = XLoadQueryFont(arwm.X.dpy, DEF_FONT);
	else return;
	if (!arwm.X.font) 
		arwm.X.font = XLoadQueryFont(arwm.X.dpy, DEF_FONT_ALT1);
	else return;
	if (!arwm.X.font) 
		arwm.X.font = XLoadQueryFont(arwm.X.dpy, DEF_FONT_ALT2);
	else return;
	if (!arwm.X.font) 
		arwm.X.font = XLoadQueryFont(arwm.X.dpy, "fixed");
	else return;
	if (!arwm.X.font)
	{
		LOG_ERROR("couldn't find a font to use: "
			"try starting with -fn fontname\n");
		exit(1);
	}
#endif /* USE_XFT */
}

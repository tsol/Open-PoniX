...
int tcpserv_init()
{
	...
	rc = WSAStartup (MAKEWORD(2, 1), &ws);

	if (rc)
	{
		printf("tcpserv: WSAStartup failure!\n");
		return INVALID_SOCKET;
	}
...
int tcpserv_connect(char *host, unsigned int port)
{
		...
		memcpy((char FAR *)&(raddr.sin_addr ), phe->h_addr, phe->h_length);
		raddr.sin_family = AF_INET;
		raddr.sin_port = htons(port);

		if (connect(tcpserv_socket, (PSOCKADDR)&raddr, sizeof(raddr)) < 0)
		{
...
int tcpserv_listen_and_accept(unsigned int port)
{
		...
		rc = bind (ls, (LPSOCKADDR)&laddr, sizeof(laddr));
        ...
        tcpserv_socket = (int)accept (ls, (LPSOCKADDR)&raddr, (int FAR *)&addrlen);
		closesocket(ls);

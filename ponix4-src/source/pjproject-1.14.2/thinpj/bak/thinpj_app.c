...
static void prn_cstate()
{
	...
	tcpserv_pf("THINPJ act='cstate', hook='%s', cur='%s', hld='%s', inc='%s'\n\r",
...
int cmd_register(char *cmd)
{
	...
	if (strstr(cmdh->get("user_url"),buf)!=NULL) // current account has same url as requested
	{
		if (info.status == 200)
		{

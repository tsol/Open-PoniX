...
make_win()
{
   ...
   gtk_window_set_title(GTK_WINDOW(win), "Imlib Config Editor");
...
add_sliders(GtkWidget * w)
{
   ...
   i3 = gtk_pixmap_new(contrast_pmap, contrast_mask);
   gtk_widget_show(i3);
   frame = gtk_aspect_frame_new("Red Levels", 0.5, 0.5, 0.0, TRUE);
   gtk_box_pack_start(GTK_BOX(box1), frame, FALSE, FALSE, 4);
   gtk_widget_show(frame);
   box = gtk_vbox_new(TRUE, 0);
   gtk_container_add(GTK_CONTAINER(frame), box);
   ...
   i3 = gtk_pixmap_new(contrast_pmap, contrast_mask);
   gtk_widget_show(i3);
   frame = gtk_aspect_frame_new("Green Levels", 0.5, 0.5, 0.0, TRUE);
   gtk_box_pack_start(GTK_BOX(box1), frame, FALSE, FALSE, 4);
   gtk_widget_show(frame);
   box = gtk_vbox_new(TRUE, 0);
   gtk_container_add(GTK_CONTAINER(frame), box);
   ...
   i3 = gtk_pixmap_new(contrast_pmap, contrast_mask);
   gtk_widget_show(i3);
   frame = gtk_aspect_frame_new("Blue Levels", 0.5, 0.5, 0.0, TRUE);
   gtk_box_pack_start(GTK_BOX(box1), frame, FALSE, FALSE, 4);
   gtk_widget_show(frame);
   box = gtk_vbox_new(TRUE, 0);
   gtk_container_add(GTK_CONTAINER(frame), box);
...
add_palsel(GtkWidget * w, char *title, char **value)
{
   ...
   filesel = gtk_file_selection_new("Select Palette File");
   ...
   button = gtk_button_new_with_label("Copy To");
   ...
   button = gtk_button_new_with_label("Spread To");
   gtk_box_pack_start(GTK_BOX(box3), button, TRUE, TRUE, 0);
   gtk_widget_show(button);
   ...
   button = gtk_button_new_with_label("Set Last");
   gtk_box_pack_start(GTK_BOX(box3), button, TRUE, TRUE, 0);
   gtk_widget_show(button);
...
add_loadsave()
{
   ...
   button = gtk_button_new_with_label("Save User Config");
   gtk_box_pack_start(GTK_BOX(box2), button, TRUE, TRUE, 0);
   gtk_widget_show(button);
   gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(save_user), NULL);

   button = gtk_button_new_with_label("Save System Config");
   gtk_box_pack_start(GTK_BOX(box2), button, TRUE, TRUE, 0);
   gtk_widget_show(button);
...
main(int argc, char **argv)
{
   ...
   load_imrc();
   make_win();
   add_note("Colors", "Rendering", "Display", "Color Correction");
   add_palsel(pg1, "Select Palette", &conf.palettefile);
   add_onoff(pg1, "Force Palette", &conf.paletteoverride);
   add_onoff(pg1, "Dither in Palette Mode", &conf.dither);
   add_onoff(pg1, "Fast Remapping in Palette Mode", &conf.remap);
   add_onoff(pg2, "High Quality in 15/16bpp", &conf.highquality);
   add_onoff_value(pg2, "Limit Maximum Shared memory Size (bytes)",
		   ...
		   "3   Mb", 3 * 1024 * 1024,
		   "4   Mb", 4 * 1024 * 1024);
   add_onoff(pg2, "Use Ordered Dithering (Faster)", &conf.ordered);
   add_onoff(pg2, "MIT-SHM Shared Memory Enabled", &conf.mitshm);
   add_onoff(pg2, "Shared Pixmaps", &conf.sharedpixmap);
   add_onoff(pg2, "Fast Render", &conf.fastrender);
   add_onoff(pg2, "Imagemagick & NETPBM Fallback", &conf.fallback);
   add_onoff_value(pg2, "Image Cache (bytes)",
		   ...
		   "6 Mb", 6 * 1024 * 1024,
		   "10 Mb", 10 * 1024 * 1024);
   add_onoff_value(pg2, "Pixmap Cache (bits)",
		   &conf.pixmapcache, &conf.pixmapcachesize,
		   "5120 Kbits", 5120 * 1024,
		   "10 Mbits", 10240 * 1024,
		   "20 Mbits", 20 * 1024 * 1024,
		   "40 Mbits", 40 * 1024 * 1024,
		   "60 Mbits", 60 * 1024 * 1024,
		   "100 Mbits", 100 * 1024 * 1024);
   add_onoff_visual(pg3, "Force Visual ID");
   add_sliders(pg4);
   add_loadsave();
   ready();
   up = 1;

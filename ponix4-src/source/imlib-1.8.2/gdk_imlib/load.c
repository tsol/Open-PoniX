...
unsigned char      *
_gdk_imlib_LoadJPEG(FILE * f, int *w, int *h)
{
...
#ifdef HAVE_LIBPNG
unsigned char      *
_gdk_imlib_LoadPNG(FILE * f, int *w, int *h, int *t)
{
  png_structp         png_ptr;
  png_infop           info_ptr;
...
#ifdef HAVE_LIBTIFF
unsigned char      *
_gdk_imlib_LoadTIFF(FILE *f, char *file, int *w, int *h, int *t)
{
  TIFF               *tif;
...
#ifdef HAVE_LIBGIF
unsigned char      *
_gdk_imlib_LoadGIF(FILE *f, int *w, int *h, int *t)
{
...
unsigned char      *
_gdk_imlib_LoadBMP(FILE *file, int *w, int *h, int *t)
{
  unsigned char      *data, *ptr;
...
unsigned char      *
_gdk_imlib_LoadXPM(FILE *file, int *w, int *h, int *t)
{
...
unsigned char      *
_gdk_imlib_LoadPPM(FILE * f, int *w, int *h)
{
  int                 done;
  unsigned char      *ptr;
  unsigned char       chr;
...
gdk_imlib_load_image(char *file)
{
  ...
  if (!eim && !data)
	...
	case 2:
	  if (p)
	    data = _gdk_imlib_LoadJPEG(p, &w, &h);
	  break;
#endif
#ifdef HAVE_LIBPNG
	case 1:

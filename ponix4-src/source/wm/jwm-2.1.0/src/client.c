...
void SetClientLayer(ClientNode *np, unsigned int layer) {
   ...
   /* harry */
   Warning("Client %s setting layer=%d", np->name, layer);                  
...
void SetClientFullScreen(ClientNode *np, int fullScreen) {
   ...
   /* harry */
   Warning("Client %s requested FULLSCREEN=%d", np->name, fullScreen);
   return;                  
...
*/
      ...
      JXMoveResizeWindow(display, np->window, 0, 0, sp->width, sp->height);
      JXMoveResizeWindow(display, np->parent, 0, 0, sp->width, sp->height);
/*
...
/* /harry */
      ...
      JXMoveResizeWindow(display, np->window, west, north, np->width, np->height);
   ...
   for(;;) {
      ...
      for(np = nodes[layer]; np; np = np->next) {
         if((np->state.status & (STAT_MAPPED | STAT_SHADED))
            ...
            /* harry */
            Warning("Stacking '%s' layer='%d', index='%d'", np->name, layer, index);

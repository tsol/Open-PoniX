...
void Signal() {
   ...
   /* harry - this function added by me. poll X server for mouse position rather then rely on internal mousex, mousey */
   GetFreshMousePosition(&x, &y);
...
void HandleMapRequest(const XMapEvent *event) {
   ...
   /* harry */
   Warning("Map request from w=%d",event->window);

...
#                     --enable-radeon --enable-libkms"
     ...
     build mesa mesa "--enable-gallium-llvm=no --enable-64-bit=no --with-gallium-drivers=no \
     		      --disable-glut --enable-gallium --disable-gallium-intel \
     		      --disable-gallium-llvm --with-gallium-drivers="" \
     		      --disable-glu --enable-dri --disable-gbm --disable-xvmc \
     		      --disable-vdpau --disable-gallium-gbm --disable-llvm-shared-libs \
     		      --disable-egl --disable-shared-glapi --disable-glx --with-dri-drivers="" \
     		      --disable-glx-tls --enable-dri3 --disable-gles1 --disable-gles2  --enable-gallium-egl"
...
build_xserver() {
    build xserver "" "--with-vendor-web=http://www.t-sol.ru/ --with-builder-addr=ponix@tsol.spb.ru --with-os-name=PoniX-4 \
                      ...
                      --disable-aiglx --disable-xephyr --disable-xnest --enable-xorg --disable-xwin \
                      --enable-docs=no --enable-devel-docs=no --disable-glx-tls \
                      --disable-dmx --disable-xvfb --disable-glx --enable-dbe --disable-glamor"
...
#           build driver xf86-video-i740 #patch for I740_Sync required
            build driver xf86-video-intel "--enable-uxa --enable-kms-only --enable-sna --disable-xvmc"
	    ;;
    esac
...
#    build driver xf86-video-qxl		#p4  depend spice-protocol >= 0.12.0
    ...
    build driver xf86-video-ati "--disable-glamor" 
    build driver xf86-video-cirrus	
    build driver xf86-video-dummy	
    build driver xf86-video-fbdev	
    build driver xf86-video-glide	#error glide library not found (use for Voodoo I/II)
...
#    build driver xf86-video-nv		#deprecated, new is nouveau 
    ...
    build driver xf86-video-vmware	#need xatracker from Mesa source (manual copied *.h files and create xatracker.pc) 
...
###    build_doc
    build_proto	
    build util makedepend	# Required by mesa and depends to xproto
    build_lib
    build_mesa
